...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort pyrtshm/
	$(ENV_PREFIX)black -l 79 pyrtshm/
...
test: lint protobuf        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v -s --full-trace --cov-config .coveragerc --cov=pyrtshm -l --tb=short --maxfail=1 tests/
...
release: protobuf          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > pyrtshm/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add pyrtshm/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@echo "pyrtshm = 'pyrtshm.__main__:main'" >> pyproject.toml
	...
	@echo "Please run 'poetry shell' or 'poetry run pyrtshm'"
